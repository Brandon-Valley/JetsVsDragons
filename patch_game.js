export function patchGame(source) {
  let text = source;

  function replaceRequired(oldText, newText, label) {
    if (!text.includes(oldText)) {
      throw new Error('Missing game patch anchor: ' + label);
    }
...
`;
  ...
  for (const item of required) {
    if (!text.includes(item)) throw new Error('Patch validation failed: ' + item);
  }
