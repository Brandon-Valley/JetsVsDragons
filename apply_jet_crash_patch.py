...
marker = '// JET_EJECTION_SEQUENCE_V2'
...
replace_once(
    ...
    "let musicEnabled = true;\ntry { musicEnabled = localStorage.getItem('jetsVsDragonsMusicEnabled') !== 'false'; } catch (e) {}\n\n// JET_EJECTION_SEQUENCE_V2\nlet jetDeathSequence = null;\nlet internetSfxPromise = null;",
    'death sequence state'
)

replace_once(
    "  for (const s of supplyDrops) scene.remove(s.group);\n  dragons = [];",
    "  for (const s of supplyDrops) scene.remove(s.group);\n  if (jetDeathSequence && jetDeathSequence.group) scene.remove(jetDeathSequence.group);\n  jetDeathSequence = null;\n  dragons = [];",
    'reset cleanup'
)
...
replace_once(
    ...
    "  if (musicNextBtn) musicNextBtn.addEventListener('click', () => nextGameMusic(musicShouldPlay && !paused && !gameOver));",
    'next song guard'
)
...
replace_once(
    ...
    function resumeGameMusic() {
      if (!musicShouldPlay || !musicEnabled || gameOver) return;
      if (!musicWidget || !musicReady) return;
      try {
...
function createEjectedPilot() {
  ...
  const torso = new THREE.Mesh(new THREE.CapsuleGeometry(0.38, 0.92, 5, 10), suitMat);
  torso.position.set(0, 0.85, -0.16);
  ...
  visor.position.set(0, 1.78, -0.43);
  ...
  for (const side of [-1, 1]) {
    ...
    arm.position.set(side * 0.49, 0.83, -0.11);
    arm.rotation.z = side * -0.34;
    arm.castShadow = true;
    group.add(arm);

    const leg = new THREE.Mesh(new THREE.CapsuleGeometry(0.14, 0.72, 4, 8), suitMat);
    leg.position.set(side * 0.24, -0.42, -0.08);
...
replace_once(
    ...
    death_functions + 'function completeTargetMode() {',
...
checks = {
    ...
    'pause music': 'pauseGameMusic();' in text and 'resumeGameMusic();' in text,
    'player jet mountain crash': "beginJetDeath('Mountain impact')" in text,
    'player dragon bounce': 'Dragon bounced off mountain' in text,
    'enemy jet crash': 'Enemy Jet Crashed' in text,
    'enemy dragon bounce': 'dragonPreviousPos' in text,
    'pilot': 'function createEjectedPilot()' in text,
    'parachute': 'function openEjectionParachute(sequence)' in text,
    'delayed result': 'sequence.time >= 2.7' in text,
}
failed = [name for name, ok in checks.items() if not ok]
if failed:
    raise SystemExit('Validation failed: ' + ', '.join(failed))

print('Patch validation passed: ' + ', '.join(checks))
